...
CREATE VIEW get_all_categories AS
SELECT
	id_category AS id,
	name
FROM categories
...
CREATE VIEW get_all_monsters AS
SELECT 
	m.id_monster, 
    c.name AS category, 
    m.name, 
...
CREATE VIEW get_all_loots_by_monsters AS
SELECT 
	m.name AS monster, 
    l.name AS loot 
...
CREATE VIEW get_all_locations AS 
SELECT 
	id_location AS id,
    name
FROM locations
...
CREATE VIEW get_all_locations_by_monster AS
SELECT 
	m.name AS monster, 
	l.name AS location
...
CREATE VIEW get_all_variations AS
SELECT 
...
CREATE VIEW get_all_tools_type AS
SELECT
	id_tool_type AS id,
    name
FROM tools_type
...
CREATE VIEW get_all_weaknesses_by_monster AS
SELECT 
	m.name AS monster, 
	t.name AS weakness 
...
CREATE VIEW get_all_inmunities_by_monster AS
SELECT 
	m.name AS monster, 
	t.name AS inmunity
